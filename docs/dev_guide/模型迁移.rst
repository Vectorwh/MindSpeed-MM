...
精度对齐是模型迁移的核心验证环节，旨在确保从GPU平台迁移至MindSpeed-MM中的模型在 **功能正确性、数值精度和训练收敛性** 三个维度上与原始模型保持一致，其核心流程如下所示：

.. image:: ../_static/dev_guide/migration/precision.png

1. **原仓在GPU/NPU上精度对齐** ：固定所有随机量后（seed、数据顺序、dropout等），对齐 **原仓GPU与NPU** 精度。此步骤验证算子精度，为MindSpeed-MM模型提供基准参考。
2. **原仓和MM在NPU上的单卡减层对齐** ：以 **原仓NPU** 训练结果为基准，对齐减少模型层数、单卡训练时的精度，确保模型结构迁移正确。
3. **原仓和MM在NPU上的多卡分布式训练对齐** ：以 **原仓NPU** 训练结果为基准，对齐整网、多卡分布式训练时的精度，验证分布式策略的正确性。
